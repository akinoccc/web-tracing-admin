        ...
        <CardContent>
          <form @submit.prevent="updateProject" class="space-y-4">
            ...
            <div class="space-y-2">
              ...
              <div class="flex items-center space-x-2">
                <Input
                  :value="projectStore.currentProject.appKey"
                  readonly
                  class="bg-muted"
                />
...
app.use(WebTracing, {
  dsn: '{{ sdkEndpoint }}',
  appKey: '{{ projectStore.currentProject.appKey }}',
  appName: '{{ projectStore.currentProject.name }}',
  debug: true,
  pv: true,
  performance: true,
  error: true,
      ...
      <Card class="w-full max-w-md">
        ...
        <CardFooter class="flex justify-between">
          ...
          <Button
            ...
            :disabled="confirmDialogType === 'delete' && confirmInput !== projectStore.currentProject.name"
...
const copyAppKey = () => {
  ...
  navigator.clipboard.writeText(projectStore.currentProject.appKey)
    .then(() => {
...
const exportData = async () => {
  ...
  try {
    ...
    a.href = url
    a.download = `${projectStore.currentProject.name}-data.json`
...
const handleConfirmAction = async () => {
  ...
  try {
    if (confirmDialogType.value === 'clear') {
      ...
      // 删除项目
      const success = await projectStore.deleteProject(projectStore.currentProject.id)
      if (success) {
        router.push('/')
      }
    }
